...
function dragDropChange(args: any) {
    ...
    if (args.checked) { gantt.allowTaskbarDragAndDrop = true; }
    else { gantt.allowTaskbarDragAndDrop = false; }
}

function overlapChange(args: any) {
    let gantt = (document.getElementsByClassName('e-gantt')[0] as any).ej2_instances[0];
    
    if (args.checked) { gantt.allowTaskbarOverlap = true; }
    else { gantt.allowTaskbarOverlap = false; }
}